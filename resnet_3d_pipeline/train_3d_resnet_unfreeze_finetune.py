...
from utils.model_utils import train_dual_paths,val_dual_paths,save_results,getscore
import pandas as pd
import math
import time
import pdb
...
model_depth = 101
n_classes = 1
checkpoint_path = '/data/ccusr/xinyug/lung/cura_crypto/resnet_3d_pipeline/eval/resnet101/dual_checkpoints_partial/fold_3.pth.tar'
...
cls_model = dual_resnet_cls_eval(encoder_nodule,encoder_lung)
model_params = torch.load(checkpoint_path)
cls_model.load_state_dict(model_params['state_dict'])
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
cls_model = cls_model.to(device)
cls_model.unfreezemodels(9)

# train classifer
print('finetune the whole model ... ')
# set up the training
weight = torch.tensor([2.8]).to(device)
criterion = nn.BCEWithLogitsLoss(pos_weight = weight)
optimizer = optim.SGD(filter(lambda p: p.requires_grad, cls_model.parameters()), lr = 0.0001, momentum = 0.9, weight_decay=0.01)
...
max_auc = 0
save_obj = save_results([],[],[],[],[],[],'resnet101_1125_finetune_2',fold_name)
